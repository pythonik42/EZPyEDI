...
class Outmessage(message.Message):
    ...
    def _formatAfield(self, value, grammarfield, record):
    ...
    def _formatDfield(self, value, grammarfield, record):
        try:
            lenght = len(value)
            if lenght==6:
    ...
    def _formatTfield(self, value, grammarfield, record):
        try:
            lenght = len(value)
            if lenght==4:
    ...
    def _formatfield(self, value, grammarfield, record):
        ...
        if grammarfield[BFORMAT] == 'A':
            value = self._formatAfield(value, grammarfield, record)
        elif grammarfield[BFORMAT] == 'D':
            value = self._formatDfield(value, grammarfield, record)
        elif grammarfield[BFORMAT] == 'T':
            value = self._formatTfield(value, grammarfield, record)
        else:   #numerics
...
class x12(var):
    ...
    def _formatAfield(self, value, grammarfield, record):
        if grammarfield[FORMAT] == 'AR':    #if field format is alfanumeric right aligned
            value = value.rjust(grammarfield[MINLENGTH])
            # print 'Format A field %s = "%s" (%s, %s) RJUST' % (grammarfield[ID], value, grammarfield[MINLENGTH], grammarfield[LENGTH])
        else:
            value = value.ljust(grammarfield[MINLENGTH])    #add spaces (left, because A-field is left aligned)
            # print 'Format A field %s = "%s" (%s, %s) LJUST' % (grammarfield[ID], value, grammarfield[MINLENGTH], grammarfield[LENGTH])
        valuelength=len(value)
        if valuelength > grammarfield[LENGTH]:
            raise botslib.OutMessageError(_(u'record "$mpath" field "$field" too big (max $max): "$content".'),field=grammarfield[ID],content=value,mpath=record[MPATH],max=grammarfield[LENGTH])
        return value
