...
class filereport(botsmodel):
    ...
    class Meta:
        db_table = 'filereport'
        verbose_name = _(u'filereport')
        unique_together = (("idta","reportidta"),)
        ordering = ['idta', 'reportidta']
    def __unicode__(self):
        return unicode(self.idta) + u' ' + unicode(self.reportidta)
...
class ta(botsmodel):
    ...
    class Meta:
        db_table = 'ta'
        verbose_name = _(u'EDI Documents')
    
    def get_absolute_url(self):
        if self.botskey:
            # build docKey from self.botskey => '%s|%s_%s' % (templateName, partnerID, docID)
            ediDocInfo = self.botskey
            (ediDocType, ediDocID) = ediDocInfo.split('|')
            (partnerID, docID) = ediDocID.split('_')
            docKey = '%s__%s_%s' % (ediDocType, partnerID, docID)
            return "/filer/?action=viewdoc&edidoc=%s" % docKey
        else:
            return None
