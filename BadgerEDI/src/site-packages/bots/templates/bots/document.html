...
<form action="{{ form.action }}" method="post">
    ...
    <table id="botstable">
        ...
        <thead>
            <tr>
                <th></th>
                <th><button class="botsheader" type="submit" name="order" value="statust" />{% trans 'Phase' %}</button></th>
                ...
                <th><button class="botsheader" type="submit" name="order" value="confirmed" />{% trans 'Confirm' %}</button></th>
        ...
        {% for row in queryset.object_list %}
            <tr class="{% cycle 'row1' 'row2' %}">
                ...
                <td><a href="{{row.get_absolute_url}}" class="botslink" target="_blank">{{row.botskey}}</a></td>
                <td>{{row.reference}}</td>
