...
def envelope(ta_info,ta_list):
    ...
    if not ta_info['envelope']: 
        ...
        try:    #see if the user is using scripted enveloping
            DEBUG_BOTS_IMPORTS = botsglobal.ini.get('settings', 'debug_bots_imports', False)
            if DEBUG_BOTS_IMPORTS:
                botsglobal.logger.debug(u'(try) to read user envelopescript editype "%s", envelope "%s".',
...
class x12(Envelope):
    ...
    def run(self):
        ...
        #mapping overrules. 
        if self.ta_info['testindicator'] and self.ta_info['testindicator']!='P':
        ...
        self.out.put({'BOTSID':'ISA',
                        ...
                        'ISA02':self.ta_info['ISA02'].ljust(10),
                        'ISA03':self.ta_info['ISA03'],
                        'ISA04':self.ta_info['ISA04'].ljust(10),
                        'ISA05':self.ta_info['ISA05'],
                        'ISA06':ISA06.ljust(15),
                        'ISA07':self.ta_info['ISA07'],
                        'ISA08':ISA08.ljust(15),
                        'ISA09':ISA09date,
                        'ISA10':time.strftime('%H%M'),
                        'ISA11':self.ta_info['ISA11'],      #if ISA version > 00403, replaced by repetition separator
        ...
        # user specified GS08 always overrides the default for 997s
        GS08 = self.ta_info.get('GS08', self.ta_info['messagetype'][3:])
        if GS08[:6]<'004010':
        ...
        if self.ta_info['version']<='00403':        # boundary error (< should have been <=) corrected by JKnaus
