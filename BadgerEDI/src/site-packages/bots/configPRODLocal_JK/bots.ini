...
ftptimeout = 180
#botsreplacechar can be used as replacement character for incoming or outgoing messages; set syntax parameters checkcharsetin and checkcharsetout using code 'botsreplace'. Default: ~. ('space' can not be set expicitly).
#botsreplacechar =
#sendreportiferror : send a report by mail if errors occured. default= False (never send)
#sendreportiferror = True
#disable edi_OrderTracking table updates (default False)
#this is for testing XML2EDI mappings against Jomar PRD DB w/o corrupting OrderTracking state in PRD DB
disableordertrackingupdates = True
#
# Logging and debug level config
#
...
log_file_number = 10
# should bots import attempts be logged (default is False)
# debug_bots_imports = True
#
# Other options for logging
#
#debug: if True, errors include trace. Default False
debug = True
#debugengine: if True, engine errors include trace. Default False, so just info messages.
debugengine = True
...
mappingdebug = False
#
# Function specific options for debug info. 
# Note: These option do not use the logging system, info goes to console.
#
...
# site root URL for BEDI-1
webroot = http://BEDI-1:8099/
# site root URL for DEEPBLUE PROD_Local EDI server
# webroot = http://DEEPBLUE:8099/
# site root URL for DEEPBLUE TEST_Local EDI server
# webroot = http://DEEPBLUE:8088/

#
# override the default botslogo for different server execution environments
#
# for TEST_Local development and testing on DEEPBLUE against Jomar TST DB
#botslogo=/media/images/botslogo-DEV.png
# once deployed to TST server, running against Jomar TST DB
#botslogo=/media/images/botslogo-TEST.png
# for PROD_Local testing on DEEPBLUE against Jomar PRD DB
botslogo=/media/images/botslogo-PRDLocal.png
# once deployed to PROD on BEDI-1, running against Jomar PRD DB
#botslogo=/media/images/botslogo.png

# BEDIS admin site port
# standard TEST server port
# port = 8088
# standard PROD server port
port = 8099
...
#
#botssys: internal storage of database, archive, etc (spool). Default: botssys
botssys = botssys
#usersys: user script files, grammars, mappings, etc. Default: usersys
usersys = usersys
#ediDataRoot: root of EDI incoming/outgoing/bots data/log files
ediDataRoot = EDI
#
...
unok = iso8859_9
unow = utf_8
#unox is defined as: Code extension technique as defined by ISO 2022 utilising the escape techniques 
#in accordance with ISO 2375.#as far as I can see there are several forms of ISO 2022. 
unox = iso2022_jp
unoy = utf_8
