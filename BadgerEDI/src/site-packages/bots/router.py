...
def routedispatcher(routestorun,type=None):
    ...
    for route in routestorun:
        ...
        for routedict in botslib.query('''SELECT idroute     ,
            ...
            bannerDict = dict(routedict)
            bannerDict['translateindText'] = (routedict['translateind'] or '--') and '> TRANS >'
            routeBanner = """>>>> PROCESSING ROUTE   %(idroute)s[%(seq)s]
                       CHANNELS   %(fromchannel)s >---%(translateindText)s---> %(tochannel)s
                 PROCESSING EDI   %(fromeditype)s.%(frommessagetype)s >---%(translateindText)s---> %(toeditype)s.%(tomessagetype)s
                  TO CHANNEL IF   FROM_TP=%(frompartner_tochannel_id)s or TO_TP=%(topartner_tochannel_id)s\n"""
            botsglobal.logger.info(routeBanner % bannerDict)
            botslib.setrouteid(routedict['idroute'])
            foundroute=True
            router(routedict)
            botslib.setrouteid('')
            botsglobal.logger.debug('%s<<<<\n', u'-'*76)
        if not foundroute:
...
def router(routedict):
    ...
    DEBUG_BOTS_ROUTER = botsglobal.ini.get('settings', 'debug_bots_router', True)      
    ...
    if routedict['fromchannel']:     #do incoming part of route: in-communication; set ready for translation; translate
        ...
        if DEBUG_BOTS_ROUTER:
            botsglobal.logger.debug('Route| No action for incoming part of route (no from-channnel)')
    ...
    if routedict['translateind']:
        ...
        if DEBUG_BOTS_ROUTER:
            botsglobal.logger.debug('Route| Translating from EDI %s.%s to %s.%s',
                                    routedict['fromeditype'], 
                                    routedict['frommessagetype'] or '-', 
                                    routedict['toeditype'], 
                                    routedict['tomessagetype'] or '-')
    ...
    if routedict['tochannel']:   #do outgoing part of route
        ...
        if routedict['frompartner_tochannel_id']:   #use frompartner_tochannel in where-clause of query (partner/group dependent outchannel
            ...
            wherestring += """ AND (frompartner=%(frompartner_tochannel_id)s 
                                    OR frompartner in (SELECT from_partner_id FROM partnergroup WHERE to_partner_id =%(frompartner_tochannel_id)s))"""
        ...
        if routedict['topartner_tochannel_id']:   #use topartner_tochannel in where-clause of query (partner/group dependent outchannel
            ...
            wherestring += """ AND (topartner=%(topartner_tochannel_id)s
                                    OR topartner in (SELECT from_partner_id FROM partnergroup WHERE to_partner_id=%(topartner_tochannel_id)s))"""
        
        toset={'tochannel':routedict['tochannel'],'status':FILEOUT}
        if DEBUG_BOTS_ROUTER:
            botsglobal.logger.debug('Route| Pass those docs WHERE %s', wherestring % towhere)
        botslib.addinfocore(change=toset,where=towhere,wherestring=wherestring)
        
        if not routedict['defer']:   #do outgoing part of route
            if DEBUG_BOTS_ROUTER:
                botsglobal.logger.debug('Route| Perform outgoing communication.')
            ...
            if DEBUG_BOTS_ROUTER:
                botsglobal.logger.debug('Route| Route DEFERs outgoing communication')
            
    else:
        if DEBUG_BOTS_ROUTER:
            botsglobal.logger.debug('Route| No action for outgoing part of route (no out-channnel)')
